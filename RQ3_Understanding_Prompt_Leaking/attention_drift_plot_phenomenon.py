...
class AttentionAnalyse(object):
    ...
    def debug_input_slices(self):
        ...
        show_slice("User Prompt", self._user_slice)
    ...
    def attention_layerwise_tokens(self, top_k_tokens=2, save_dir="./attn", attention_type = "mean", name_prefix=""):
        ...
        for token_idx, output_token_id in enumerate(target_token_ids):
            ...
            for layer_idx in range(num_layers):
                ...
                '''
                # Mean attention to each segment
                sp_score = norm_attn[self._system_prompt_slice].mean().item()
                dp_score = norm_attn[self._defense_slice].mean().item()
                up_score = norm_attn[self._user_slice].mean().item()
                '''
                attn_across_layers.append([dp_score, up_score])
            ...
            ax = sns.heatmap(token_attn_matrix, xticklabels=list(range(1,num_layers+1)), yticklabels=['Defensive Instruction', 'Adversarial Query'],
                        cmap='YlGnBu', annot=False, fmt=".2f")

            cbar = ax.collections[0].colorbar
            cbar.ax.tick_params(labelsize=16)
            token_text = self.tokenizer.decode([output_token_id]).strip()
            ...
            filename = os.path.join(save_dir, f"{name_prefix}_token{token_idx + 1}.png")
            plt.savefig(filename)
            filename = os.path.join(save_dir, f"{name_prefix}_token{token_idx + 1}.pdf")
    ...
    def attention_calc(self, attention_type, k=5):
        ...
        last_layer_attn = attentions[-1][0]  # shape: (num_heads, tgt_len, src_len)
        attn = last_layer_attn.mean(0)[self._target_slice]  # shape: (tgt_len, src_len)
        ...
        data = [attn_defense.cpu().tolist()[2:], attn_user.cpu().tolist()[2:]]
        ...
        print(x_labels)
        y_labels = ['Defensive Instruction', 'Adversarial Query']
...
if __name__ == "__main__":
    ...
    user_prompts_json = 'data/attack/adversarial_query.json'
    defensive_instruction_json = 'data/defense/generated.json' 
    output_csv = 'attention_results.csv'
    output_dir = 'attn'    

    os.makedirs(output_dir, exist_ok=True)
    os.makedirs("./attn_token_map", exist_ok=True)
    os.makedirs("./attn_attention_sink", exist_ok=True)
    os.makedirs("./attention_result_txt", exist_ok=True)
    ...
    for sys_index, system_prompt in enumerate(system_prompts):
        for def_index, defense_suff in enumerate(defensive_instruction):
            for user_index, user_prompt in enumerate(user_prompts):
                ...
                plt.figure(figsize=(18, 3))
                ax = sns.heatmap(data, cmap='YlGnBu', annot=False, fmt=".2f", xticklabels=x_labels, yticklabels=y_labels)
                ...
                plt.tight_layout()
                plt.savefig(f'./attn/heatMap_{sys_index}_{def_index}_{user_index}.pdf', bbox_inches="tight")
                plt.savefig(f'./attn/heatMap_{sys_index}_{def_index}_{user_index}.png')

                filename_prefix = f"heatMap_{sys_index}_{def_index}_{user_index}"
                attentionAnalyse.attention_layerwise_tokens(top_k_tokens=2, save_dir="./attn_token_map", attention_type = "topk", name_prefix=filename_prefix)
  
                attentionAnalyse.get_segment_token_attention_sink(
                    top_k=5,
                    plot=True,
                    save_path=f"attn_attention_sink/segment_token_sink_{sys_index}_{def_index}_{user_index}.png"
                )

                filename_prefix = f"sys{sys_index}_def{def_index}_usr{user_index}"
                output_path = f"./attention_result_txt/{filename_prefix}.txt"
                attentionAnalyse.save_full_prompt_output(save_path=output_path)
