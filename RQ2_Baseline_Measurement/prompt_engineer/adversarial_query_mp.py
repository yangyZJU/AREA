...
'''
This code implements prompt engineering-based defense methods for RQ2. It is a multi-process implementation designed to generate model responses to adversarial queries under different defense methods.
...
Repeated Prefix:
python adversarial_query_mp.py --model meta-llama/Llama-3.1-8B-Instruct --resume --defense prefix --num_processes 2

Fake Prompt:
python adversarial_query_mp.py --model meta-llama/Llama-3.1-8B-Instruct --resume --defense fakeone --num_processes 2

Random Insertion:
python adversarial_query_mp.py --model meta-llama/Llama-3.1-8B-Instruct --resume --defense insert --num_processes 2

Only Local Lookup:
python adversarial_query_mp.py --model meta-llama/Llama-3.1-8B-Instruct --resume --defense locallook --num_processes 2
'''
import warnings
...
def parse_args():
    ...
    parser.add_argument("--attack_file", type=str, default="Adversarial_Subset/llama3_adversarial_query.json")
    parser.add_argument("--output_csv", type=str, default="results/llama3_attack.csv")
...
# Representative defense strategies based on prompting engineering
highh_ppl_phrase_ls = [
...
def worker_process(process_id, args, task_list, processed_set):
    ...
    with open(temp_output, "w", encoding="utf-8-sig", newline="") as out_f:
        ...
        for task_idx, (row_idx, prompt_idx, system_prompt_raw, user_prompt) in enumerate(task_list):
...
def main():
    ...
    logger.info(f"Loaded {len(user_prompt_list)} adversarial queries")
    
    df = pd.read_csv(args.csv)
    logger.info(f"Loaded {len(df)} system prompts")
    ...
    for row_idx, row in df.iterrows():
        ...
        for prompt_idx, user_prompt in enumerate(user_prompt_list):
            all_tasks.append((row_idx, prompt_idx, system_prompt_raw, user_prompt))
