...
def evaluate_csv(
    ...
    for _, row in tqdm(
        ...
        sys_p = str(row["raw_system_prompt"]) if pd.notna(row["raw_system_prompt"]) else ""
        out_t = str(row["output_text"]) if pd.notna(row["output_text"]) else ""
    ...
    print(f"Average similarity (raw_system_prompt vs output_text): {avg_sim:.6f}")
