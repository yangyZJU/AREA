...
def evaluate_one_csv_writeback(
    *,
    ...
    required_cols = ["raw_system_prompt", "output_text"]
    ...
    def score_one_row(idx):
        raw_system_prompt = df.at[idx, "raw_system_prompt"]
        output_text = df.at[idx, "output_text"]

        if is_empty(raw_system_prompt) or is_empty(output_text):
            return idx, None

        true_p = str(raw_system_prompt)
        guess_p = str(output_text)
...
if __name__ == "__main__":
    df, stats = evaluate_one_csv_writeback(
        csv_path="mistral_results/mistral_attack.csv",
