...
def gpt41_rate_answer_quality(
    ...
    *,
    ...
    """
    Judge: given (system_prompt as predefined instructions, user_query, assistant_answer),
...
def evaluate_one_csv_answer_quality_writeback(
    *,
    ...
    required_cols = ["system_prompt", "user_query", "final_response"]
    score_col = "RUS"
    ...
    def score_one_row(idx):
        ...
        user_query = df.at[idx, "user_query"]
        assistant_answer = df.at[idx, "final_response"]

        if is_empty(system_prompt) or is_empty(user_query) or is_empty(assistant_answer):
            return idx, None

        r = cached_rate(str(system_prompt), str(user_query), str(assistant_answer))
        return idx, r
