...
def load_system_prompts_from_csv(csv_path: str) -> dict:
    """
    ...
    Returns:
        dict: Dictionary mapping id to system_prompt
    """
    ...
    for _, row in df.iterrows():
        systems_data[str(row['id'])] = str(row['system_prompt'])
...
def get_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="ID of the system prompt to use from the CSV file"
    )
...
def obfuscate_hard_prompt(
    ...
    """
    Obfuscate the system prompt using hard prompt obfuscation.
    ...
    Args:
        ...
        obf_sys_prompt_len (int) - Length of the obfuscated system prompt
        optimizer_iter (int) - Number of optimization iterations
        token_windows (List[List[int]]) - List of token indices for each token window
        topk (int) - Top-k value for candidate generation
        search_width (int) - Search width for candidate generation
        n_replace (int) - Number of token replacements for candidate generation
        ce_weight (float) - Weight for cross-entropy loss
        kl_weight (float) - Weight for KL divergence loss
    ...
    for token_window_idx, token_window_indices in enumerate(token_windows):
        ...
        for iteration in range(optimizer_iter):
            ...
            logger.debug(f"Current obfuscated system prompt: {sys_prompt_obf_str}")
...
def main(
    ...
    Args:
        ...
        systems_csv (str) - Path to CSV file containing system prompts.
        system_id (str) - ID of the system prompt to use from the CSV file.
    ...
    logger.info(f"Loading system prompts from CSV: {systems_csv}")
    try:
        systems_data = load_system_prompts_from_csv(systems_csv)
        if system_id not in systems_data:
            logger.error(f"System ID '{system_id}' not found in CSV file")
            return
        task_system_prompt = systems_data[system_id]
        logger.info(f"Using system prompt for ID '{system_id}': {task_system_prompt[:100]}...")
    except Exception as e:
        logger.exception(f"Failed to load system prompts from CSV: {e}")
        return
    ...
    try:
        train_samples, test_samples, _ = load_and_prepare_dataset(
            dataset_name,
            dataset_size,
            task_hints,
            seed,
