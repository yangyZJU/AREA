...
def main(
    ...
    """
    Main function for running prompt extraction attack.

    Args:
        results_dir (str) - Path to the directory where obfuscate.py saved its results.
        extraction_prompts_file (str) - Path to the file containing extraction prompts.
        batch_size (int) - Batch size for generating model outputs during attack.
        output_filename (str) - Filename for the output file.
        conventional (bool | None) - If True, use the conventional system prompt in params.json.
        system_prompt (str | None) - Specify a custom system prompt directly as a string.
        tensor_file (str | None) - Specify a tensor file to load the system prompt from.
...
if __name__ == "__main__":
    setup_logging('prompt_extraction.log', 'DEBUG')
    logger = logging.getLogger(__name__)
